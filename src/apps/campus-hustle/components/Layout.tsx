...
import { Home, List, ShoppingBag, PenTool, User, Moon, Sun, Menu, X, ArrowLeft } from 'lucide-react';
...
interface LayoutProps {
    ...
    onBack?: () => void; // Added onBack for integration
}

const Layout: React.FC<LayoutProps> = ({ currentView, setView, children, onBack }) => {
    ...
    return (
        <div className="min-h-screen bg-ceramic dark:bg-obsidian transition-colors duration-300 flex overflow-hidden">
            ...
            {/* Sidebar - Desktop */}
            <aside className="hidden md:flex flex-col w-64 p-6 fixed h-full z-10 transition-colors">
                <div className="flex items-center gap-3 mb-10 px-2">
                    {onBack && (
                        <NeuButton onClick={onBack} className="!p-2 !rounded-lg !w-10 !h-10 flex items-center justify-center mr-2">
                            <ArrowLeft size={20} />
                        </NeuButton>
                    )}
                ...
                <div className="pt-6">
                    <NeuButton onClick={toggleTheme} className="w-full">
            ...
            <div className="md:hidden fixed top-0 left-0 right-0 z-20 p-4 flex justify-between items-center bg-ceramic/90 dark:bg-obsidian/90 backdrop-blur-md pointer-events-none">
                <div className="flex items-center gap-2 pointer-events-auto">
                    {onBack && (
                        <NeuButton onClick={onBack} className="!p-2 !rounded-lg !w-10 !h-10 flex items-center justify-center">
                            <ArrowLeft size={18} />
                        </NeuButton>
                    )}
                    <div className="w-8 h-8 rounded-lg bg-emerald-500 flex items-center justify-center shadow-lg shadow-emerald-500/30">
                ...
                <div className="pointer-events-auto">
                    <NeuButton onClick={toggleTheme} className="!p-2 !rounded-full w-10 h-10 flex items-center justify-center">
