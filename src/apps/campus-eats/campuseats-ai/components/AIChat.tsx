...
export const AIChat: React.FC<AIChatProps> = ({ isOpen, onClose, restaurants }) => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Hey! I\'m your Campus Eats concierge. Hungry? Ask me for recommendations based on your budget or cravings! 🥑' }
  ]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 backdrop-blur-sm p-4">
      <NeuCard className="w-full max-w-md h-[600px] flex flex-col overflow-hidden relative border-4 border-ceramic-100 dark:border-obsidian-900">
        ...
        {/* Chat Area */}
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar bg-ceramic-200 dark:bg-obsidian-800/50">
            ...
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`
                ...
                ${msg.role === 'user' 
                   ...
                   : 'bg-ceramic-100 dark:bg-obsidian-900 text-slate-700 dark:text-slate-300 rounded-bl-none shadow-sm'
                }
              `}>
                {msg.text}
              </div>
