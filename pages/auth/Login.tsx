            ...
            >
                ...
                <div className="bg-ceramic-base dark:bg-obsidian-surface p-8 rounded-3xl shadow-[10px_10px_20px_#bebebe,-10px_-10px_20px_#ffffff] dark:shadow-[10px_10px_20px_#151618,-10px_-10px_20px_#35363e]">
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        ...
                        {/* Hide Google OAuth on iOS standalone — redirect flow doesn't work in PWA webview */}
                        {(() => {
                            const isIOSStandalone = (window.matchMedia('(display-mode: standalone)').matches || (window.navigator as any).standalone) && /iPhone|iPad|iPod/.test(navigator.userAgent);
                            if (isIOSStandalone) {
                                return (
                                    <p className="text-center text-xs text-slate-400 py-2">
                                        Google Sign-In is available from Safari
                                    </p>
                                );
                            }
                            return (
